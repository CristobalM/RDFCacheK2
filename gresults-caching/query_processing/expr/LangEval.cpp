...
LangEval::eval_resource(const ExprEval::row_t &row) {
  ...
  auto language_tag = ExprProcessorPersistentData::get().extract_language_tag(concrete_resource.value);
  return std::make_unique<StringLiteralResource>(std::move(language_tag), ExprDataType::EDT_STRING);
}
