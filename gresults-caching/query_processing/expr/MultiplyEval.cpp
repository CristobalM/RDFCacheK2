...
MultiplyEval::eval_resource(const ExprEval::row_t &row) {
  ...
  if(first->is_double() || second->is_double()){
    return std::make_unique<DoubleResource>(first->get_double() * second->get_double());
  }
  if(first->is_float() || second->is_float()){
    return std::make_unique<FloatResource>(first->get_float() * second->get_float());
  }

  return std::make_unique<IntegerResource>(first->get_float() * second->get_float());
}
