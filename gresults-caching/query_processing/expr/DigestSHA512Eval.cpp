...
DigestSHA512Eval::eval_resource(const ExprEval::row_t &row) {
  ...
  if (child_resource->is_concrete()) {
    ...
    result = sha512_human_readable_lowercase(literal_content);
  } else if (child_resource->is_string_literal()) {
    result =
        sha512_human_readable_lowercase(child_resource->get_literal_string());
  } else if (child_resource->can_cast_to_literal_string()) {
    result = sha512_human_readable_lowercase(
