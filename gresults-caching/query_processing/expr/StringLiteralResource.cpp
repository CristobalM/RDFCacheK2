...
bool StringLiteralResource::is_string_literal() const {
  return true;
}
...
ExprDataType StringLiteralResource::get_datatype() const {
  return data_type;
}
...
bool StringLiteralResource::operator==(const TermResource &rhs) const {
  if(!rhs.is_concrete() || !rhs.is_string_literal()) return false;
  ...
  if(rhs.is_concrete()){
    ...
    if(rhs_resource.resource_type != RDFResourceType::RDF_TYPE_LITERAL) return false;
    auto rhs_datatype = ExprProcessorPersistentData::get().extract_data_type_from_string(rhs_resource.value);
    if(data_type != rhs_datatype) return false;
    auto rhs_value = ExprProcessorPersistentData::get().extract_literal_content_from_string(rhs_resource.value);
