...
bool IsBlankEval::eval(const std::vector<unsigned long> &row) const{
  ...
  if(node.expr_case() != proto_msg::ExprNode::kTermNode || node.term_node().term_type() != proto_msg::TermType::VARIABLE)
    throw std::runtime_error("Invalid operand for IS_BLANK(), expected a variable");
  ...
  return resource.resource_type == RDFResourceType::RDF_TYPE_BLANK;
}
