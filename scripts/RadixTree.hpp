...
public:
  ...
  void insert(const std::string &key, DataT &value) {
    ...
    raxInsert(inner_rt, reinterpret_cast<const unsigned char *>(key.c_str()),
              key.size(), reinterpret_cast<void *>(held_ptr), nullptr);
  }

  LookupResult &lookup(const std::string &key) {
    void *v_data =
        raxFind(inner_rt, reinterpret_cast<const unsigned char *>(key.c_str()),
                key.size());
  ...
  void remove(const std::string &key) {
    raxRemove(inner_rt, reinterpret_cast<const unsigned char *>(key.c_str()),
              key.size(), nullptr);
  }
