...
TEST(update_log_test, can_compact_log_one_insert_one_delete) {
    ...
    auto fetched_after_compaction = pcm.get_predicates_index_cache().fetch_k2tree(predicate_id_1);
    ASSERT_TRUE(fetched_after_compaction.exists());
    ASSERT_EQ(fetched_after_compaction.get_mutable().size(), 0);

    auto after_compaction_scanner = fetched_after_compaction.get_mutable().create_full_scanner();
