...
public:
  ...
  QueryResultIteratorHolder run_query(const proto_msg::SparqlNode &query_tree);
...
private:
  ...
  process_project_node(const proto_msg::ProjectNode &project_node,
                       std::shared_ptr<VarBindingQProc> var_binding_qproc);
  std::shared_ptr<QueryIterator> process_left_join_node(
      const proto_msg::LeftJoinNode &left_join_node,
      const std::shared_ptr<VarBindingQProc> &var_binding_qproc);
  std::shared_ptr<QueryIterator> process_optional_node(
      const proto_msg::OptionalNode &optional_node,
      const std::shared_ptr<VarBindingQProc> &var_binding_qproc);

  void remove_repeated_rows(ResultTable &input_table);
  std::shared_ptr<QueryIterator> process_filter_node(
      const proto_msg::FilterNode &node,
      const std::shared_ptr<VarBindingQProc> &var_binding_qproc);
  std::shared_ptr<QueryIterator> process_extend_node(
  ...
  create_var_pos_mapping(QueryIterator &input_it);
  std::shared_ptr<QueryIterator>
