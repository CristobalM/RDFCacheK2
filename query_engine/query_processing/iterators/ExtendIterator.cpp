...
#include "ExtendIterator.hpp"
#include <TimeControl.hpp>
ExtendIterator::ExtendIterator(
    ...
    : QueryIterator(time_control), input_it(std::move(input_it)),
      eval_data(std::move(eval_data)), var_bindings(std::move(var_bindings)),
      headers(build_headers()), mapping_original(build_mapping_original()),
      mapping_extra(build_mapping_extra()) {}
bool ExtendIterator::has_next() { return input_it->has_next(); }
std::vector<unsigned long> ExtendIterator::next() { return next_concrete(); }
std::vector<unsigned long> &ExtendIterator::get_headers() { return headers; }
void ExtendIterator::reset_iterator() { input_it->reset_iterator(); }
std::vector<unsigned long> ExtendIterator::build_headers() {
...
std::vector<unsigned long> ExtendIterator::next_concrete() {
...
std::vector<unsigned long> ExtendIterator::build_mapping_original() {
...
std::vector<unsigned long> ExtendIterator::build_mapping_extra() {
  std::vector<unsigned long> result;
...
ExtendIterator::get_transformed_row(std::vector<unsigned long> &input_row) {
...
unsigned long ExtendIterator::get_rdf_resource_id_value(RDFResource &resource) {
