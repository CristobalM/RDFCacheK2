...
#include "LeftOuterJoinIterator.hpp"
#include <query_processing/QueryProcessor.hpp>
bool LeftOuterJoinIterator::has_next() { return next_available; }

std::vector<unsigned long> LeftOuterJoinIterator::next() {
  return next_concrete();
}
std::vector<unsigned long> &LeftOuterJoinIterator::get_headers() {
  return headers;
}
void LeftOuterJoinIterator::reset_iterator() {
...
LeftOuterJoinIterator::LeftOuterJoinIterator(
...
void LeftOuterJoinIterator::build_headers(
...
bool LeftOuterJoinIterator::create_var_binding_qproc_if_needed(
...
std::vector<unsigned long> LeftOuterJoinIterator::next_concrete() {
  next_available = false;
...
LeftOuterJoinIterator::get_pos_map(std::vector<unsigned long> &from,
...
void LeftOuterJoinIterator::map_rows_to_holder(
...
void LeftOuterJoinIterator::fill_right_rows_with_zero() {
  for (auto p : right_positions) {
    tmp_holder[p] = 0;
  }
}
std::vector<unsigned long> LeftOuterJoinIterator::build_right_positions(
