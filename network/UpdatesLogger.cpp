...
void UpdatesLogger::compact_logs() {
    ...
    for (const auto &p: offsets_map) {
        ...
        std::vector<K2TreeUpdates> updates = {
                K2TreeUpdates(compacted->predicate_id, compacted->add_update.get(), compacted->del_update.get())};
