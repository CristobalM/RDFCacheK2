...
public:
  ...
  : std::runtime_error("Not found predicate with index " + std::to_string(predicate_index))
  {}
};
...
public:
  explicit PredicateAlreadyExists(const ulong predicate_index)
          : std::runtime_error("There is already one predicate with the index " + std::to_string(predicate_index))
  {}
};

class RootNotFound : std::runtime_error{
public:
  RootNotFound() : std::runtime_error("Root block was not found when reading binary string"){}
};
