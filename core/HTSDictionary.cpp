...
#include "HTSDictionary.hpp"
#include "serialization_util.hpp"

HTSDictionary::HTSDictionary(
...
void HTSDictionary::serialize_dict(
...
HTSDictionary::deserialize_dict(const std::string &fname) {
...
std::unordered_map<RDFResource, unsigned long, NaiveHash>
HTSDictionary::create_reverse(std::vector<RDFResource> &input) {
...
void HTSDictionary::save(const std::string &res_fname) {
  serialize_dict(resources_extra, res_fname);
}

HTSDictionary
HTSDictionary::load(const std::string &res_fname) {
  auto res = deserialize_dict(res_fname);
  auto reverse_res = create_reverse(res);
  return HTSDictionary(std::move(res), std::move(reverse_res));
}

void HTSDictionary::add_resource(RDFResource resource) {
  reverse_resources[resource] = resources_extra.size();
  resources_extra.push_back(std::move(resource));
}

unsigned long HTSDictionary::locate_resource(
...
RDFResource
HTSDictionary::extract_resource(unsigned long id) const {
  long next_id = static_cast<long>(id) - 1;
  assert(id > 0 && next_id >= 0 && next_id < (long)resources_extra.size());
  return resources_extra[next_id];
}

size_t HTSDictionary::size() const {
  return resources_extra.size();
}
void HTSDictionary::merge_with_extra_dict(
        HTSDictionary &other_dict) {
...
void HTSDictionary::serialize(std::ostream &ofs) {
  uint64_t size = 0;
...
HTSDictionary
HTSDictionary::deserialize(std::istream &ifs) {
  uint64_t size = read_u64(ifs);
  std::vector<char> buffer(size, 0);
  ifs.read(buffer.data(), size);
