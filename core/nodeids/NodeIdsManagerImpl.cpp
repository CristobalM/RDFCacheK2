...
{
  this->nodes_map->restore(
      this->log_fh->get_reader(std::ios::binary),
...
long NodeIdsManagerImpl::find_last_assigned() {
  if(nodes_map->get_last_assigned() == NOT_FOUND_NODEID) return nodes_sequence->get_last_assigned();
