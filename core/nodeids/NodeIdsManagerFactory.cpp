...
std::unique_ptr<NodeIdsManager>
NodeIdsManagerFactory::create(std::unique_ptr<I_FileRWHandler> &&plain_ni_fh,
                              std::unique_ptr<I_FileRWHandler> &&mapped_ni_fh) {
  return std::make_unique<NodeIdsManagerImpl>(std::move(plain_ni_fh),
                                              std::move(mapped_ni_fh));
}
